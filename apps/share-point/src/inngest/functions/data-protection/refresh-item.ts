...
import { getAllItemPermissions } from '@/connectors/microsoft/share-point/permissions';
import { getItem } from '@/connectors/microsoft/share-point/item';
...
export const refreshItem = inngest.createFunction(
  {
    id: 'share-point-refresh-data-protection-objects',
    concurrency: {
  ...
  { event: 'share-point/data_protection.refresh_object.requested' },
  async ({ event, step }) => {
    const {
      id: itemId,
      organisationId,
