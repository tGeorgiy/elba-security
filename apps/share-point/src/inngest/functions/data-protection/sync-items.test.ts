...
import * as itemsConnector from '@/connectors/microsoft/share-point/items';
...
const setup = createInngestFunctionMock(syncItems, 'share-point/items.sync.triggered');
...
describe('sync-items', () => {
    ...
    if (folders.length) {
      ...
      for (let i = 0; i < folders.length; i++) {
        ...
        expect(step.waitForEvent).nthCalledWith(i + 1, `wait-for-folders-complete-${folder?.id}`, {
          event: 'share-point/foder-items.sync.completed',
  ...
  test('should finalize the sync when there is no next page', async () => {
    ...
    expect(step.sendEvent).toBeCalledWith('folders-sync-complete', {
      name: 'share-point/foder-items.sync.completed',
      data: {
        organisationId: organisation.id,
        folderId,
      },
  ...
  test('should call elba.dataProtection.updateObjects', async () => {
    ...
    expect(step.sendEvent).toBeCalledWith('items-sync-complete', {
      name: 'share-point/items.sync.completed',
      data: {
        organisationId: organisation.id,
        driveId,
      },
